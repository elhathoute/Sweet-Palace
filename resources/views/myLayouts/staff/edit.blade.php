...
<div class="container-fluid">
    ...
    <div class="card shadow my-5">
        <div class="card-header py-3 d-flex align-items-center justify-content-between">
            <h6 class="m-0 fw-bold">Update Staff</h6>
            <a href="{{url('myLayouts/staff')}}" class="text-decoration-none btn btn-success">View All</a>
        </div>

        <div class="card-body">
            <form method="post" action="{{url('myLayouts/staff/' . $staff->id)}}"  enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="departement_id" name="departement_id">
                        ...
                        @foreach ($departements as $dt)
                            <option @if($staff->departement_id==$dt->id) selected @endif value="{{$dt->id}}">{{$dt->title}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="bio" name="bio" rows="4">{{$staff->bio}}</textarea>
                    @error('bio')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="salary_amount" name="salary_amount" value="{{$staff->salary_amount}}">
                    @error('salary_amount')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
                </div>
